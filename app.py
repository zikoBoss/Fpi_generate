...
def pArSeRoOm(iT):
    """تحليل بيانات Protobuf بدون مكتبة خارجية"""
    try:
...
def generate_accounts(region, name_prefix, count, max_allowed=3):
    region = region.upper()
...
def generate_get():
    ...
    if not all([number, region, name_prefix]):
        return jsonify({"error": "Missing parameters: number, region, name"}), 400
